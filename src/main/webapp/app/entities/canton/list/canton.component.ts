...
import { EntityArrayResponseType, CantonService } from '../service/canton.service';
...
export class CantonComponent implements OnInit {
  ...
  trackId = (_index: number, item: ICanton): number => this.cantonService.getCantonIdentifier(item);
  ...
  protected onResponseSuccess(response: EntityArrayResponseType): void {
    ...
    this.cantons = dataFromBody;
  }

  protected fillComponentAttributesFromResponseBody(data: ICanton[] | null): ICanton[] {
    return data ?? [];
  }
  ...
  protected queryBackend(page?: number, predicate?: string, ascending?: boolean): Observable<EntityArrayResponseType> {
    ...
    return this.cantonService.query(queryObject).pipe(tap(() => (this.isLoading = false)));
  }
